...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  // Check for new OpenClaw config first, then legacy
  try {
    ...
    if (!checkLogs.stdout?.includes('ok')) {
      ...
      const legacyCheckProc = await sandbox.startProcess('test -f /root/.clawdbot/clawdbot.json && echo "ok"');
      await waitForProcess(legacyCheckProc, 5000);
      const legacyCheckLogs = await legacyCheckProc.getLogs();
      if (!legacyCheckLogs.stdout?.includes('ok')) {
        return {
          success: false,
  ...
  // Sync both OpenClaw (.openclaw) and legacy (.clawdbot) directories
